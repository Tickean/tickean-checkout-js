...
type CheckoutMode = "headless" | "elements" | "composed";
  ...
  return (
    <div className="grid two">
      ...
      <aside className="card">
        ...
        paymentInfo?.nextAction?.type === "display_instructions" ? (
          <pre style={{ fontSize: 12, overflow: "auto" }}>
            {JSON.stringify(
              paymentInfo.nextAction?.type === "display_instructions"
                ? paymentInfo.nextAction.paymentInstructions
                : paymentInfo.paymentInstructions,
              null,
              2,
            )}
...
function ComposedCheckout() {
  return (
    <div className="grid two">
      <section className="card">
        <TicketSelector locale="es-AR" appearance="default" />
        <DiscountField locale="es-AR" appearance="default" />
        <BuyerVerification locale="es-AR" appearance="default" />
      </section>
      <aside className="card">
        <OrderSummary locale="es-AR" appearance="default" />
        <PaymentElement
          locale="es-AR"
          appearance="default"
          paymentMethod="TRANSFER"
...
function resolveInitialMode(): CheckoutMode {
  if (String(process.env.NEXT_PUBLIC_TICKEAN_ELEMENTS || "false") === "true") {
    return "elements";
  }
  return "headless";
}

function useCheckoutMode(): [CheckoutMode, (mode: CheckoutMode) => void] {
  const [mode, setMode] = useState<CheckoutMode>(resolveInitialMode);
...
export default function CheckoutClient() {
  const [mode, setMode] = useCheckoutMode();
    ...
    <main>
      <header style={{ marginBottom: 24 }}>
        ...
        <p className="muted" style={{ marginTop: 8, fontSize: 14 }}>
          Modo: <strong>{mode}</strong> — cambiá con los botones o{" "}
          <code>?mode=headless|composed|elements</code>
        </p>
      </header>
      ...
      >
        {mode === "elements" ? (
          <TickeanCheckout appearance="default" locale="es-AR" />
        ) : mode === "composed" ? (
          <ComposedCheckout />
        ) : (
          <DemoCheckout />
